...
from backend.src.agents.llm_advisor import LLMAdvisor
from backend.src.simulation.monitor import Monitor

# API App (mounted later)
api_app = FastAPI(title="Hospital Agentic AI API")

# Setup CORS on sub-app as well for direct hits
...
from backend.src.simulation.patient import Patient
...
# Set CORS on parental app to cover API and proxy
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "http://localhost:3000",
...
@app.middleware("http")
async def log_parent_requests(request: Request, call_next):
    print(f"[DEBUG] Parent Request: {request.method} {request.url.path}")
    response = await call_next(request)
    print(f"[DEBUG] Parent Response Status: {response.status_code}")
    return response
