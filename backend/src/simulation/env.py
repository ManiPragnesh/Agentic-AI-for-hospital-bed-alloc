...
from backend.src.simulation.monitor import Monitor
from backend.src.agents.admission_agent import AdmissionAgent
from backend.src.agents.resource_agent import ResourceAgent
from backend.src.agents.discharge_agent import DischargeAgent

from backend.src.agents.staff_agent import StaffAgent

from backend.src.agents.staff_agent import StaffAgent
from backend.src.simulation.department import Department
...
class HospitalSim:
    ...
    def department_allocation_loop(self, dept_name):
        ...
        while True:
            ...
            if len(dept.queue) > 0 and not dept.is_full():
                # Priority allocation via ResourceAgent
                patient = self.resource_agent.allocate_bed(dept)
                if not patient:
                    yield self.env.timeout(1)
                    continue
                ...
                if dept.get_staff_ratio() < 4.0: 
                    ...
                    if patient in dept.queue:
                        # Start Treatment (which will request the bed)
    ...
    def process_stay(self, patient, dept):
        ...
        with dept.beds.request() as req:
            ...
            # Admit patient (Moves from queue to patients list)
            dept.admit_patient(patient)
