...
class Patient:
    ...
    def log_event(self, time, event, message, details=None):
        # Local History
        payload = {
            'time': time,
            'event': event,
        ...
        # Centralized Log
        if self.monitor:
             # Merge details with basic tracking info
             full_details = {'patient_id': self.id, 'criticality': self.criticality}
             if details:
                  full_details.update(details)
             self.monitor.log_event(time, event, message, full_details)
